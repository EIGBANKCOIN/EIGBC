...
CAmount CMasternode::GetMasternodePayment(CAmount& nBlockValue)
{
    return nBlockValue * 0 / 100;
}

CAmount CMasternode::GetTreasuryPayment(CAmount& nBlockValue)
{
    return nBlockValue * Params().GetConsensus().nTreasuryPercentage / 100;
}
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 EIGBankCoin tx got MASTERNODE_MIN_CONFIRMATIONS
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        ...
        int nConfHeight = pMNIndex->nHeight + MASTERNODE_MIN_CONFIRMATIONS - 1; // block for 1000 EIGBankCoin tx -> 1 confirmation
