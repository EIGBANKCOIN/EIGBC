...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all EIGBankCoin specific functionality (Masternodes) (0-1, default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2023-%i The EIGBankCoin Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that EIGBankCoin is running in a usable environment with all
...
#endif
    LogPrintf("EIGBankCoin version %s (%s)\n", version_string, CLIENT_DATE);
}
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. EIGBankCoin is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single EIGBankCoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. EIGBankCoin is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //eigbankcoin specific: spork DB's
                ...
                // EIGBankCoin: load previous sessions sporks if we have them.
