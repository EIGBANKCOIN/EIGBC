...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("EIGBC client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("EIGBC"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("EIGBC"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
