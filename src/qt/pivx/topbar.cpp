                                                        ...
                                                        ui(new Ui::TopBar)
{
    ...
    ui->pushButtonConf->setButtonText("conf");
...
void TopBar::onBtnConfClicked()
{
    ...
    if (!GUIUtil::openConfigfile())
        inform(tr("Unable to open eigbankcoin.conf with default application"));
}
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("EIGBC")
    updateDisplayUnit();
