...
%{_sbindir}/semanage port -a -t pivx_port_t -p tcp 15116
%{_sbindir}/semanage port -a -t pivx_port_t -p tcp 15115
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 15116
	%{_sbindir}/semanage port -d -p tcp 15115
